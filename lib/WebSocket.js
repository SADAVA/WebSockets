...
const EventListener		= require("@sadava/eventlistener");
	...
	writeBuffer(buffer) {
		if(this.status !== this.OPEN) {
			this.emit("error", new Error(
				"writeBuffer: client already closed"
			));
...
function pairWithClient(self, socket, clientKey) {
	...
	socket.writeBuffer({}, Buffer.from("HTTP/1.1 101 Switching Protocols\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nSec-WebSocket-Accept: " + clientPairKey + "\r\n\r\n"));

	let clientID = self.clients.push(client) - 1;
	client.id = clientID;
